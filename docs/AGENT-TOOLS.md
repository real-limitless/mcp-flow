...
If a namespaced tool is gated, the same `tools/call` (or `mf_call_tool`) **waits** up to 3 minutes for a human in Admin → Approvals (or a Web Push Approve/Deny on `notify_approve`). Timeout returns `reason: "authz_timeout"` — tell the user to approve, then retry. Deny returns `authz_denied`. Discovery metas (`mf_status`, `mf_list_tools`, …) are never gated. Raise the harness `tools/call` timeout to at least the rule wait (SDK default is often 60s). Install Admin as a PWA on a phone for notifications. Details: [AUTHZ-STEP-UP.md](./AUTHZ-STEP-UP.md).
